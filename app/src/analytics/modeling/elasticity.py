...
class Elasticity:
    ...
    def get_cross_elasticity_matrix(self, type: str = "matrix"):
        if type == "matrix":
            return self.cross_elasticity_matrix
        elif type == "dict":
            return {
                (i, j): self.cross_elasticity_matrix.loc[i, j]
                for i in self.cross_elasticity_matrix.index
                for j in self.cross_elasticity_matrix.columns
            }