...
def get_remainder_data(message, user_id):
    try:
        remainder_extractor = model.with_structured_output(RecordatorioBase)
        ...
        return Recordatorio(
            ...
            fecha_recordatorio=recordatorio_create.fecha_recordatorio.replace(tzinfo=tz_cdmx),
            recordatorio=recordatorio_create.recordatorio
        )
